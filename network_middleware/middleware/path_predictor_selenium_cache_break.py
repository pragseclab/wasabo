...
def http_get(url, headers=None, redirects=True):
    driver = get_chrome_driver()

    urlparts = urlparse(url)
    path = urlparts.path
    cacheable_extensions = [".css", ".js", ".txt", ".png", ".jpg", ".ico"]

    if(path in cacheable_extensions and "msgID" not in urlparts.query):
        params = {"msgID" : ''.join(random.choice(string.ascii_letters) for x in range(10))}
        req = PreparedRequest()
    ...
    try:
        ...
        request = None
        url_to_find = url.lower()
        for r in driver.requests:
            if(r.url.lower() == url_to_find):
                if(r.response == None):
                    print("no response to request: ", r.url)
                    continue
                elif(r.response.status_code == 200 or not redirects):
                    request = r
                    break
        ...
        return (request.response.status_code, dict(request.response.headers), request.response.body)
    except Exception as e:
        logging.error(e)
    finally:
        if(driver != None):
...
#                             headers=headers, verify=False, allow_redirects=redirects)
...
class WebPathPredictor:
    ...
    def request(self, flow):
        urlparts = urlparse(flow.request.pretty_url)
        domain = urlparts.hostname

        # First make a request for the given URL and return the value if it exists
        status_code, headers, content = http_get(flow.request.pretty_url, redirects=False)
        ...
        for path in paths:
            ...
            status_code, headers, content = http_get(flow.request.pretty_url, headers=dict(flow.request.headers))
